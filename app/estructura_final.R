...
ui <- dashboardPage(
  ...
  dashboardBody(
    ...
    # Condición para mostrar los filtros solo en las pestañas específicas
    conditionalPanel(
      condition = "input.sidebarMenu != 'datos'",  # Mostrar los filtros excepto en la pestaña "Datos"
      fluidRow(
        # Filtros
      ...
      tabItem(tabName = "datos",
              fluidRow(
                box(
                  ...
                  DT::DTOutput("tableData")  # Cambiado a DT::DTOutput
                )
              )
      )
    )
...
server <- function(input, output, session) {
  ...
  datos_filtrados <- reactive({
    ...
    if (input$level_selection == "Nacional") {
      # Filtrar datos a nivel nacional
      if (input$migration_filter == "Todos") {
        # Supongamos que tienes un data frame llamado `data`
        # Asegúrate de que `data` esté cargado y tenga las columnas necesarias
        ...
        resultados <- list(
          pyramid_population_result = pyramid_population_nacional(Venezuelan_Migrants),
          marital_status_result = marital_status_nacional(Venezuelan_Migrants),
          sex_result = sex_nacional(Venezuelan_Migrants),
          education_achieved_result = education_achieved_nacional(Venezuelan_Migrants),
          income_by_education_result = income_by_education_nacional(Venezuelan_Migrants),
          group_variables_result = group_variables_nacional(Venezuelan_Migrants),
          job_type_result = job_type_nacional(Venezuelan_Migrants),
          calcular_tipo_vivienda_result = calcular_tipo_vivienda_nacional(Venezuelan_Migrants),
          home_conditions_result = calcular_condiciones_hogar_nacional(Venezuelan_Migrants),
          calcular_acceso_salud_result = calcular_acceso_salud_nacional(Venezuelan_Migrants),
          calcular_afiliacion_salud_result = calcular_afiliacion_salud_nacional(Venezuelan_Migrants)
        )
      }
    } else {
      ...
      if (input$migration_filter == "Todos") {
        ...
        resultados <- list(
          pyramid_population_result = piramide_poblaciona_dep(Venezuelan_Migrants, input$department_selection),
          marital_status_result = marital_status_dep(Venezuelan_Migrants, input$department_selection),
          sex_result = sexo_departamento_dep(Venezuelan_Migrants, input$department_selection),
          education_achieved_result = nivel_educacion_dep(Venezuelan_Migrants, input$department_selection),
          income_by_education_result = ingreso_por_educacion_dep(Venezuelan_Migrants, input$department_selection),
          group_variables_result = estadisticas_laborales_dep(Venezuelan_Migrants, input$department_selection),
          job_type_result = tipo_trabajo_dep(Venezuelan_Migrants, input$department_selection),
          calcular_tipo_vivienda_result = tipo_vivienda_dep(Venezuelan_Migrants, input$department_selection),
          home_conditions_result = calcular_condiciones_hogar_dep(Venezuelan_Migrants, input$department_selection),
          calcular_acceso_salud_result = cobertura_salud_dep(Venezuelan_Migrants, input$department_selection),
          calcular_afiliacion_salud_result = afiliacion_salud_dep(Venezuelan_Migrants, input$department_selection)
        )
      }
    }
