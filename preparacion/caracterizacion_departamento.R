...
marital_status_dep <- function(geih, department_selection) {
  geih[DPTO == department_selection & !is.na(P6070), .(personas = sum(FEX_C18, na.rm = TRUE) / 7), by = .(DPTO, P6070)]
}
...
nivel_educacion_dep <- function(geih, department_selection) {
  education_achieved_dept_m <- geih[DPTO == department_selection & !is.na(P3042), .(personas = sum(FEX_C18, na.rm = TRUE) / 7), by = .(DPTO, P3042)]
...
tipo_trabajo_dep <- function(geih, department_selection) {
  t_job_dep_m <- geih[DPTO == department_selection & !is.na(P6430), .(personas = sum(FEX_C18, na.rm = TRUE) / 7), by = .(DPTO, P6430)]
  return(t_job_dep_m)
}
...
tipo_vivienda_dep <- function(geih, department_selection) {
  housing_type_dept_m <- geih[DPTO == department_selection & !is.na(P5090), .(personas = sum(FEX_C18, na.rm = TRUE) / 7), by = .(DPTO, P5090)]
...
afiliacion_salud_dep <- function(geih, department_selection) {
  health_affiliation_dept_m <- geih[DPTO == department_selection & !is.na(P6100), .(personas = sum(FEX_C18, na.rm = TRUE) / 7), by = .(DPTO, P6100)]
