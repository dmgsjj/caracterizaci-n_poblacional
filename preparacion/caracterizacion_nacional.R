...
marital_status_nacional <- function(geih) {
  geih[!is.na(P6070), .(personas = sum(FEX_C18, na.rm = TRUE) / 7), by = .(P6070)]
}
...
education_achieved_nacional <- function(geih) {
  geih[!is.na(P3042), .(personas = sum(FEX_C18, na.rm = TRUE) / 7), by = .(P3042)]
}
...
job_type_nacional <- function(geih) {
  geih[!is.na(P6430), .(personas = sum(FEX_C18, na.rm = TRUE) / 7), by = .(P6430)]
}
...
calcular_afiliacion_salud_nacional <- function(geih) {
  health_affiliation_nacional <- geih[!is.na(P6100), .(personas = sum(FEX_C18, na.rm = TRUE) / 7), by = .(P6100)]
