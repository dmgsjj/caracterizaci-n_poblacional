...
marital_status_dep <- function(data, department_selection ) {
  data[DPTO== department_selection &!is.na(P6070), .(personas = sum(FEX_C18, na.rm = TRUE) / 7), by = .(DPTO, P6070)]
}



# Sexo por departamento
sexo_departamento_dep <- function(data, department_selection) {
  sex_dept_m <- data[DPTO== department_selection, .(personas = sum(FEX_C18, na.rm = TRUE) / 7), by = .(DPTO, P3271)]
...
nivel_educacion_dep <- function(data, department_selection) {
  education_achieved_dept_m <- data[DPTO== department_selection &!is.na(P3042), .(personas = sum(FEX_C18, na.rm = TRUE) / 7), by = .(DPTO, P3042)]
...
ingreso_por_educacion_dep <- function(data, department_selection) {
  income_by_education_dept_m <- data[DPTO== department_selection & !is.na(P3042) & !is.na(INGLABO), .(ingreso = mean(INGLABO, na.rm = TRUE)), by = .(DPTO, P3042)]
...
estadisticas_laborales_dep <- function(data, department_selection) {
  factor_expansion <- data[DPTO== department_selection & !is.na(P3271), .(factor_expansion = sum(FEX_C18) / 7), by = .(DPTO, P3271)]
  ocupados <- data[DPTO== department_selection & !is.na(P3271), .(ocupados = sum(OCI * FEX_C18, na.rm = TRUE) / 7), by = .( DPTO, P3271)]
  desocupados <- data[DPTO== department_selection & !is.na(P3271), .(desocupados = sum(DSI * FEX_C18, na.rm = TRUE) / 7), by = .(DPTO, P3271)]
  poblacion_edad_trabajar <- data[DPTO== department_selection & P6040 >= 15, .(poblacion_edad_trabajar = sum(FEX_C18, na.rm = TRUE) / 7), by = .(DPTO, P3271)]
...
tipo_trabajo_dep <- function(data, department_selection) {
  t_job_dep_m <- data[DPTO== department_selection & !is.na(P6430), .(personas = sum(FEX_C18, na.rm = TRUE) / 7), by = .(DPTO, P6430)]
  return(t_job_dep_m)
}
...
tipo_vivienda_dep <- function(data, department_selection) {
  housing_type_dept_m <- data[DPTO== department_selection & !is.na(P5090), .(personas = sum(FEX_C18, na.rm = TRUE) / 7), by = .(DPTO, P5090)]
...
calcular_condiciones_hogar_dep  <- function(data, department_selection) {
  electrical_energy_nacional <- data[DPTO == department_selection &! is.na(P4030S1), .(energia_electrica = sum(PT * FEX_C18, na.rm = TRUE) / 7), by = .(DPTO, P4030S1)]
  ...
  natural_gas_nacional <- data[DPTO == department_selection & !is.na(P4030S2), .(gas_natural = sum(PT * FEX_C18, na.rm = TRUE) / 7), by = .(DPTO, P4030S2)]
  ...
  sewer_nacional <- data[DPTO == department_selection &!is.na(P4030S3), .(alcantarillado = sum(PT * FEX_C18, na.rm = TRUE) / 7), by = .(DPTO, P4030S3)]
  ...
  aqueduct_nacional <- data[DPTO == department_selection & !is.na(P4030S5), .(acueducto = sum(PT * FEX_C18, na.rm = TRUE) / 7), by = .(DPTO, P4030S5)]
...
cobertura_salud_dep <- function(data, department_selection) {
  health_coverage_dep_m <- data[DPTO == department_selection, .(personas = sum(FEX_C18, na.rm = TRUE) / 7), by = .(DPTO, P6090)]
  return(health_coverage_dep_m)
}


# Afiliación al sistema de salud
afiliacion_salud_dep <- function(data, department_selection) {
  health_affiliation_dept_m <- data[DPTO == department_selection & !is.na(P6100), .(personas = sum(FEX_C18, na.rm = TRUE) / 7), by = .(DPTO, P6100)]
