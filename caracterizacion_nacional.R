...
pyramid_population_nacional <- function(data) {
  ...
  pyramid_nacional <- data[, .(personas = sum(FEX_C18, na.rm = TRUE) / 7), by = .(age_group, P3271)]
...
marital_status_nacional <- function(data) {
  data[!is.na(P6070), .(personas = sum(FEX_C18, na.rm = TRUE) / 7), by = .(P6070)]
}

# Sexo de la población a nivel nacional
sex_nacional <- function(data) {
  sex <- data[, .(personas = sum(FEX_C18, na.rm = TRUE) / 7), by = .(P3271)]
...
education_achieved_nacional <- function(data) {
  data[!is.na(P3042), .(personas = sum(FEX_C18, na.rm = TRUE) / 7), by = .(P3042)]
}

# Ingreso por nivel educativo a nivel nacional
income_by_education_nacional <- function(data) {
  data[!is.na(P3042) & !is.na(INGLABO), .(ingreso = mean(INGLABO, na.rm = TRUE)), by = .(P3042)]
}

######### Mercado Laboral #####

# Función para calcular variables laborales a nivel nacional
group_variables_nacional <- function(data) {
  factor_expansion <- data[!is.na(P3271), .(factor_expansion = sum(FEX_C18) / 7), by = P3271]
  
  ocupados <- data[!is.na(P3271), .(ocupados = sum(OCI * FEX_C18, na.rm = TRUE) / 7), by = P3271]
  
  desocupados <- data[!is.na(P3271), .(desocupados = sum(DSI * FEX_C18, na.rm = TRUE) / 7), by = P3271]

  poblacion_edad_trabajar <- data[P6040 >= 15, .(poblacion_edad_trabajar = sum(FEX_C18, na.rm = TRUE) / 7), by = P3271]
...
job_type_nacional <- function(data) {
  data[!is.na(P6430), .(personas = sum(FEX_C18, na.rm = TRUE) / 7), by = .(P6430)]
}

######## Vivienda #####

# Tipo de vivienda a nivel nacional
calcular_tipo_vivienda_nacional <- function(data) {
  housing_type_nacional <- data[!is.na(P5090), .(personas = sum(FEX_C18, na.rm = TRUE) / 7), by = .(P5090)]
...
calcular_condiciones_hogar_nacional  <- function(data) {
  electrical_energy_nacional <- data[, .(energia_electrica = sum(PT * FEX_C18, na.rm = TRUE) / 7), by = .(P4030S1)]
  ...
  natural_gas_nacional <- data[!is.na(P4030S2), .(gas_natural = sum(PT * FEX_C18, na.rm = TRUE) / 7), by = .(P4030S2)]
  ...
  sewer_nacional <- data[!is.na(P4030S3), .(alcantarillado = sum(PT * FEX_C18, na.rm = TRUE) / 7), by = .(P4030S3)]
  ...
  aqueduct_nacional <- data[!is.na(P4030S5), .(acueducto = sum(PT * FEX_C18, na.rm = TRUE) / 7), by = .(P4030S5)]
...
calcular_acceso_salud_nacional <- function(data) {
  health_coverage_nacional <- data[, .(personas = sum(FEX_C18, na.rm = TRUE) / 7), by = .(P6090)]
  return(health_coverage_nacional)
}

# Afiliación al sistema de salud a nivel nacional
calcular_afiliacion_salud_nacional <- function(data) {
  health_affiliation_nacional <- data[!is.na(P6100), .(personas = sum(FEX_C18, na.rm = TRUE) / 7), by = .(P6100)]
